<?php
	...
	// `city`.`cityName`,
	$aColumns = array("rowNumber","profilePic","fullName","email","RequestTime","location","document","action",);
	...
	$sTable = "
		(
			SELECT
				...
				`user`.`email`,
				`user`.`requestTime`,
				CONCAT(' <a href=\"../userDetails/?id=',`user`.`userId`,'\"  style=\"width: 30px;height: 30px;margin-right:5px;\"  data-toggle=\"tooltip\" data-theme=\"dark\" data-placement=\"right\" class=\"symbol symbol-lg-35 symbol-25 symbol-light-success\">',
					(CASE
				...
				REPLACE(REPLACE(REPLACE(`user`.`city`, '\"', ''), ']', ''), '[', '') AS `location`,
				...
				WHEN `user`.`status` = 0 
				THEN
				CONCAT('<button onclick=\"ApproveDriver(',`user`.`userId`,')\" style=\"width: 30px;height: 30px;\" title=\"approve\" data-toggle=\"tooltip\" data-theme=\"dark\" data-placement=\"right\" class=\"btn btn-icon btn-success\"><i class=\"fas fa-check\"></i></button> 
				
				<button onclick=\"changeText(',`user`.`userId`,')\" style=\"width: 30px;height: 30px;\" title=\"reject\" data-toggle=\"tooltip\" data-theme=\"dark\" data-placement=\"right\" class=\"btn btn-icon btn-danger\"><i class=\"fas fa-times\"></i></button>
				...
				END ) AS `action`
						...
						WHERE
						`user`.`delete`!='1' AND `user`.`is_driver`='0' AND `user`. `is_requested`='1'  ORDER BY `user`.`userId` DESC
