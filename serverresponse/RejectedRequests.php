...
// `city`.`cityName`,
$aColumns = array("rowNumber", "profilePic", "fullName", "email", "rejectionReason","RequestTime", "location", "document", "action",);
		...
		(
			SELECT
				...
				END ) AS `action`
						...
						WHERE
						`user`.`delete`!='1' AND `user`.`is_driver`='0' AND `user`. `is_requested`='3'  ORDER BY `user`.`userId` DESC
